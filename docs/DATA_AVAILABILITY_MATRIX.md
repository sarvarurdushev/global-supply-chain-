...
| 17 | **Observed infrastructure damage** | "Does the dataset exist?" — **NO, not as data** | The road, bridge and hospital *positions* are open (OSM/Overpass). What happened to each one is not. Post-event condition reaches the public as narrative — situation reports, cluster updates, news — not as a machine-readable state per asset id. Nepal 2015 is the best-documented case available and still has no per-bridge status feed. So `INFRA_STATE` in `src/disaster/impact.js` carries **`AT_RISK`** (this asset lies inside a modelled hazard zone) and **`UNKNOWN`**, and `STATE_BASIS` names which of the two produced every marker. The interface never colours an asset `CLOSED` on the strength of a model. Closing this would need a national road-authority damage register or an OSM-id-keyed assessment layer; neither exists for these events. |
