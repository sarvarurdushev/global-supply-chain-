...
| 13 | **Inland freight of any kind** | "Does the dataset exist?" — **NO** | Freight rail corridors, road-freight corridors, airport cargo tonnage and pipeline throughput. No open global dataset covers any of them: corridors are published per-operator in incompatible formats, airport tonnage is per-authority or IATA-licensed, and pipeline throughput is commercial. All four are LISTED in the application's layer browser under "Not Available" with what would be needed — hiding them is how a user concludes the app forgot rather than that the data does not exist. City transit (GTFS) and city road traffic ARE available and are a different thing, which the interface states in those words. |
| 14 | **Facility-level supply-chain stages** | "Does the dataset exist?" — **NO** | Extraction, processing and manufacture, plus inland distribution and the final consumer: five of the ten stages `Analyze → Trade Route` draws. Mine, well and farm locations with output are not open at global scale; refineries and smelters are company assets whose throughput is commercial; facility output is confidential; trade data stops at the border. The five are shown as explicit gaps with why and what would fix them. |
| 15 | **Basin-level water stress** | "Is it accessible?" — **NO (bulk only)** | National averages hide the distribution that matters: China's withdrawal figure averages the water-rich south with the water-scarce north, and the north is where the wheat is. WRI Aqueduct publishes water stress by river basin, which is the right resolution, and is a bulk download rather than an API. Not integrated. National indicators ARE used, labelled as national on every result. |
