...
`data/raw` was deleted and `node pipelines/run-all.mjs --force` re-run from
nothing. **Eight of the ten processed artefacts regenerated byte-identically** —
USGS events, ShakeMap contours, UNOSAT damage, Copernicus grading, NGA
infrastructure, district boundaries, OCHA exposure and WorldPop population.

The two OpenStreetMap artefacts reproduced their data exactly as well: **2,129
ways and 105,887 positions** with an identical class breakdown, and **393**
context ways. The coverage count re-measured independently and returned
**7,143 ways again** — the same figure, from a fresh set of queries.

Only two fields move between runs, and both are meant to:

| Field | Why it moves |
| --- | --- |
| `overpassBaseTimestamp` | The mirror's database snapshot advances. It records *which* database answered, which is the point of having it. |
| raw `sha256` of the context response | The response bytes differ run to run while the 393 parsed ways do not — the reader is deterministic even when the server is not. |

### The weakness the re-run exposed, and the fix

The coverage figure needs 28 extra queries to a public Overpass mirror that
answers a second concurrent query with a 504. It fails on roughly half of runs,
and on one of those the first version of the ingest **overwrote a real
measurement with `null`** — losing evidence and leaving a reader unable to tell
"never measured" from "the mirror was busy that day".

That is fixed rather than papered over. The comparison is stored as a dated
sub-object, and `resolveNetworkComparison` — pure, in `src/nepal/io/overpass.js`,
with its own tests — decides what it should say: a fresh measurement wins; a
failed run carries the previous one forward **with its original date**, never
advancing it, and marks `carriedForward: true`; a count from an artefact written
before the field existed is migrated using that artefact's own `generatedAt`;
and anything undated is refused rather than invented.

So the figure is **dated evidence about today's OpenStreetMap** rather than a
property of the 2015 snapshot, and the artefact records which run measured it.
The 2015 baseline every connectivity result rests on reproduces exactly.
